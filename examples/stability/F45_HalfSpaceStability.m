...
%%% 3. Generates Figures 4 and 5A in Skarbek, 2024 JGR.
...
%%% This script takes ~25 minutes to execute on Rob's computer: 3.7 GHz 
%%% processor, 64 GB RAM, Linux OS.

%%% dxi =  H_s_F/250 in StabilitySetup.m, which gives min(dxi) ~ L_d/40. 
...
beta = (0:0.5:90)';
% beta = [0 0.5 10 90]';
Burial_Star = [0; 0.001; 0.01; 0.1; 1];

%%% Values of a/b.
ab = 0.8;

%%% StabilityRun1 does not use the value of Length_star for 'Thrust Fault'
%%% and 'Normal Fault' geometries.
Length_star = 5; % dummy value.

%%% Thrust faults.
j = 1;
mu_0 = 0.6;
[T_T06, P_T06] = StabilityRun1(ab, Length_star, calc, geom{j}, beta, mu_0,...
    Burial_Star);

mu_0 = 0.2;
[T_T02, P_T02] = StabilityRun1(ab, Length_star, calc, geom{j}, beta,...
    mu_0, Burial_Star(1:4));

mu_0 = 1;
[T_T1, P_T1] = StabilityRun1(ab, Length_star, calc, geom{j}, beta,...
    mu_0, Burial_Star(1:4));

%%% Normal faults.
j = 2;
mu_0 = 0.6;
[T_N06, P_N06] = StabilityRun1(ab, Length_star, calc, geom{j}, beta, mu_0,...
    Burial_Star);

mu_0 = 0.2;
[T_N02, P_N02] = StabilityRun1(ab, Length_star, calc, geom{j}, beta,...
    mu_0, Burial_Star(1:4));

mu_0 = 1;
[T_N1, P_N1] = StabilityRun1(ab, Length_star, calc, geom{j}, beta,...
    mu_0, Burial_Star(1:4));
...
%%% Thrust faults.
ct = [217 83 25]/255;
t1 = plot(Burial_Star, cell2mat(T_T06{i_05,:})', ':o', 'Color', ct,...
    'MarkerEdgeColor', 'k', 'MarkerFaceColor', ct, 'MarkerSize', 10,...
    'LineWidth', 3);
drawnow
t1.MarkerHandle.LineWidth = 1;
hold on
t2 = plot(Burial_Star, cell2mat(T_T06{i_10,:})', '-o', 'Color', ct,...
    'MarkerEdgeColor', 'k', 'MarkerFaceColor', ct, 'MarkerSize', 10,...
    'LineWidth', 3);
drawnow
t2.MarkerHandle.LineWidth = 1;
t3 = plot(Burial_Star, cell2mat(T_T06{i_20,:})', '--o', 'Color', ct,...
    'MarkerEdgeColor', 'k', 'MarkerFaceColor', ct, 'MarkerSize', 10,...
    'LineWidth', 3);
drawnow
t3.MarkerHandle.LineWidth = 1;

% t2b = plot(Burial_Star, cell2mat(T_T06{i_10,:})', '-', 'Color', ct, 'LineWidth', 3);

%%% Normal faults.
cn = [114 189 255]/255;
n1 = plot(Burial_Star, cell2mat(T_N06{i_05,:})', ':s', 'Color', cn,...
    'MarkerEdgeColor', 'k', 'MarkerFaceColor', cn, 'MarkerSize', 10,...
    'LineWidth', 3);
drawnow
n1.MarkerHandle.LineWidth = 1;
n2 = plot(Burial_Star, cell2mat(T_N06{i_10,:})', '-s', 'Color', cn,...
    'MarkerEdgeColor', 'k',    'MarkerFaceColor', cn, 'MarkerSize', 10,...
    'LineWidth', 3);
drawnow
n2.MarkerHandle.LineWidth = 1;
n3 = plot(Burial_Star, cell2mat(T_N06{i_20,:})', '--s', 'Color', cn,...
    'MarkerEdgeColor', 'k', 'MarkerFaceColor', cn, 'MarkerSize', 10,...
    'LineWidth', 3);
drawnow
...
text(nexttile(2), 0.05, 0.05, 'B', 'units', 'normalized', 'FontSize', 30,...
    'Interpreter', 'latex')
