function OnFaultData = BP6QD_OnFault(SimData)
%%% This needs to be modified to include the Darcy velocity at each station.

%%% Off-fault station locations. Distance from the center of the fault [m].
C = 1e3*[-1.5; 0; 0.5; 1; 1.5; 2.5; 3.5; 5; 7.5];
...
%%% First generate a name for the station.
    if C(q) < 0
        StationName = ['fltst_strk_n', num2str(abs(C(q,1)/1e2))];
    else
        StationName = ['fltst_strk_p', num2str(abs(C(q,1)/1e2))];
    end
...
%%% station.
    StationData = [SimData.Output.Time, SimData.Output.Slip(:,i),...
        ...
        SimData.Output.PorePressure(:,i), log10(SimData.Output.State(:,i))];

    VarNames = [{'Time_s'}, {'Slip_m'}, {'SlipRate_log10ms'},...
        {'ShearStress_MPa'}, {'PorePressure_MPa'}, {'State_log10s'}];
